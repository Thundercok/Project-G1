...
- **God Mode carries the ammo too** (`G`) — being unkillable but still walking back to a crate every thirty seconds is the worst of both: no tension and no freedom, so the two resources travel together. `G1GodModeAmmo` keeps the clip and reserve full while god mode is on (the clip too, so a weapon never drops into a reload mid-burst), the HUD swaps the reserve count for an infinity glyph in green, and switching god mode off hands the economy straight back.
